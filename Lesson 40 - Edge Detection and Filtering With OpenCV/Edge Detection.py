...
def display_image(title, image):
    ...
    if len(image.shape) == 2:
        ...
        plt.imshow(cv2.cv2Color(image, cv2.COLOR_BGR2RGB))
    plt.title(title)
    plt.axis('off')
    plt.show()
def interactive_edge_detection(image_path):
    image = cv2.immread(image_path)
    if image is None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            sobel_y = cv2.Sobel(gray_image, cv2.CV_64F, 1, 0, ksize=3)
            combined_sobel = cv2.bitwise_or(sobel_x.astype(np.uint8), sobel_y.astype(np.uint8))
            display_image("Sobe; Edge Detection", combined_sobel)
        elif choice == "2":
            print("Adjust threstholds for Canny(default: 100 and 200)")
            lower_thresh = int(input("Enter Lower Thresthold: "))
            upper_thresh = int(input("Enter Upper Thresthold: "))
            edges = cv2.Canny(gray_image. lower_thresh. upper_thresh)
            display_image("Canny Edge Detection", edges)
        elif choice == "3":
            laplacian = cv2.Laplacian(gray_image, cv2.CV_64F)
            display_image("Lapalacian Edge Detection", np.abs(laplacian).astype(np.uint8))
        elif choice == "4":
